...
const CarouselComp = () => {    
    ...
    const handlers = useSwipeable({
        onSwipedLeft : () => navigate_carousel() , 
        onSwipedRight : () => navigate_carousel(false) , 
        trackMouse:true
    })
  ...
  return (
    ...
    <div {...handlers} className="relative w-full select-none" >
...

            {
                database.portfolio?.map((data , index) => (
                    <div key={index}>
                        ...
                        <div className={`${index != active ? 'hidden' : ''} md:hidden  flex flex-col-reverse items-end justify-start gap-4 pb-10`} >
                            {/* data */}
        ...
        <button onClick={() => navigate_carousel(false)} className="max-md:hidden absolute top-0 start-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" >
        ...
        <button onClick={navigate_carousel} type="button" className="max-md:hidden absolute top-0 end-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" >
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-500/30 dark:bg-gray-300/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                <svg className="w-4 h-4 text-gray-500 dark:text-gray-300 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
